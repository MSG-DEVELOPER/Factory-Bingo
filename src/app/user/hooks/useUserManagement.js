...
export const useUserManagement = () => {
  const [players, setPlayers] = useState(() => getInitialState('bingo_players', defaultPlayers));
  const [sellers, setSellers] = useState(() => getInitialState('bingo_sellers', defaultSellers));
  const [currentPlayer, setCurrentPlayer] = useState(() => getInitialState('bingo_currentPlayer', null));
  const [currentSeller, setCurrentSeller] = useState(() => getInitialState('bingo_currentSeller', null));
  ...
  useEffect(() => {
    localStorage.setItem('bingo_sellers', JSON.stringify(sellers));
  }, [sellers]);

  useEffect(() => {
    localStorage.setItem('bingo_currentPlayer', JSON.stringify(currentPlayer));
  }, [currentPlayer]);
  
  useEffect(() => {
    localStorage.setItem('bingo_currentSeller', JSON.stringify(currentSeller));
  }, [currentSeller]);
